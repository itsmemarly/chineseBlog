...
function createAdmin($request_values){
	...
	if (count($errors) == 0) {
		...
		$query = "INSERT INTO accounts (username, email, role, password, created_at, updated_at) 
...
function updateAdmin($request_values){
	...
	if (count($errors) == 0) {
		...
		$query = "UPDATE accounts SET username='$username', email='$email', role='$role', password='$password' WHERE id=$admin_id";
		mysqli_query($conn, $query);
...
function updateTopic($request_values) {
	...
	// register topic if there are no errors in the form
	if (count($errors) == 0) {
		$query = "UPDATE `topics` SET topic_name='$topic_name', topic_slug='$topic_slug' WHERE id=$topic_id";
		mysqli_query($conn, $query);
